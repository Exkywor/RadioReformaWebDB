  ...
  switch(field) {
    case "Name":
      ...
      if (isEmpty(value)) // Check if the name is empty
        ...
        return displayInputValidation(action, false, "Name", `El nombre no puede contener solo caracteres especiales`);
        ...
        else return displayInputValidation(action, false, "Name", `El nombre ${value} ya está asignado a otro programa`);
      else
        return displayInputValidation(action, true, "Name");
    case "Length":
      if (value < 1) // Check if the length is less than 1 (this is impossible due to the limit in the input, but still)
        return displayInputValidation(action, false, "Length", "La duración debe ser mayor a 0");
      else
        return displayInputValidation(action, true, "Length");
    case "Topics":
    ...
    case "Presenters":
      ...
      if (presenters.length > 0) {// Proceed if presenters is not empty, ignore and consider as valid otherwise
        if (!isAlphaNumeric(value)) // Check if it's not alphanumeric
          return displayInputValidation(action, false, "Presenters", `Los presentadores no pueden contener solo caracteres especiales`);
    ...
    case "Author":
      if (isEmpty(value)) // Check if the author is empty
        return displayInputValidation(action, false, "Author", "Debes añadir el productor del programa");
      else if (!isAlphaNumeric(value)) // Check if it's not alphanumeric
        return displayInputValidation(action, false, "Author", `El productor no puede contener solo caracteres especiales`);
      else
    ...
    case "DescriptionLong":
      if (isEmpty(value)) // Check if the descriptionLong is empty
        return displayInputValidation(action, false, "DescriptionLong", "Debes añadir una descripción del programa");
      else if (!isAlphaNumeric(value)) // Check if it's not alphanumeric
        return displayInputValidation(action, false, "DescriptionLong", `La descripción no puede contener solo caracteres especiales`);
      else
    ...
    case "monday":
      ...
      if (times[0] === ",")
        return displayInputValidation(action, false, field, `El ${field.toUpperCase()} no puede contener solo una coma`);
